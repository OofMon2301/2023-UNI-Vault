  ...
  async status() {
    ...
    if (response.status === 200) {
      ...
      if (response2.status === 503) {
        return { status: 1 /* Loading */, msg: "Inference API is being provisioned, please try again in a few seconds" };
      } else {
        return { status: 0 /* Ready */, msg: "Inference API is ready and accepting images" };
      }
    } else if (response.status === 400 || response.status === 401) {
      return { status: 4 /* Misconfigured */, msg: "Unauthorized: check your API key in the settings" };
    } else {
      console.error(response);
      return { status: 3 /* Unreachable */, msg: `Got ${response.status}: ${response.status}` };
    }
  }
};
...
var LatexOCRSettingsTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    const apiKeyInput = new import_obsidian4.Setting(containerEl).setName("Set API Key").setDesc("Hugging face API key. See https://huggingface.co/docs/api-inference/quicktour#get-your-api-token.").addText((text) => text.onChange(async (value) => {
      let key;
      if (safeStorage_default.isEncryptionAvailable()) {
...
var LatexOCR = class extends import_obsidian5.Plugin {
  async onload() {
    ...
    this.model.load();
    try {
      await fs2.promises.mkdir(path4.join(this.pluginPath, "/.clipboard_images/"));
    } catch (err) {
  ...
  async clipboardToText(editor) {
    try {
      ...
      try {
        ...
        const imgpath = path4.join(this.pluginPath, `/.clipboard_images/pasted_image.${filetype}`);
